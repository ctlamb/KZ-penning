...
---
title: "Klinse-Za Penning Demography"
author: "Clayton Lamb"
...
rmarkdown::render(here::here('kz-penningdemography.Rmd'),
...
```{r surv data stats, message=FALSE, warning=FALSE}
##inds
...
```{r surv data stats for recruit, message=FALSE, warning=FALSE}
##inds
surv.recruit <- surv%>%
...
## Calf survival
## data summary stats
```{r calf surv data stats, message=FALSE, warning=FALSE}
##clean survival data
...
```{r Avalanches extent prep, message=FALSE, warning=FALSE}
...
```{r Avalanches data, eval=FALSE, message=FALSE, warning=FALSE}
...
```{r Avalanches prep,message=FALSE, warning=FALSE}
##load rasters
...
## Plot Avalanche map
```{r Avalanches map,  message=FALSE, warning=FALSE}
raster::plot(avy)
```

##  Add avalanche data to telemetry and then to survival
```{r Avalanches to telem data,  message=FALSE, warning=FALSE}
...
```{r Avalanches and survival,  message=FALSE, warning=FALSE}
##COXPH survival
m1 <- coxph(Surv(dur, dead)~ 1 +
              frailty(id), data = surv.pen)
m2 <- coxph(Surv(dur, dead)~ PenSeason +
